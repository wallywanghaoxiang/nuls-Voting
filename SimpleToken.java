...
public class SimpleToken implements Contract, Token {
    ...
    public SimpleToken(@Required String name, @Required String symbol,@Required String md5mark,@Required String transfermark, @Required BigInteger initialAmount, @Required int decimals) {
        this.name = name;
        this.symbol = symbol;
        ...
        emit(new TransferEvent(null,Msg.sender(),transfermark, totalSupply));
    }
    ...
    @Override
    public boolean transferFrom(@Required Address from, @Required Address to,@Required String transfermark, @Required BigInteger value) {
        subtractAllowed(from, Msg.sender(), value);
        subtractBalance(from, value);
        addBalance(to, value);
        emit(new TransferEvent(from,to,transfermark, value));
        return true;
    }
    ...
    public boolean transfer(@Required Address to, @Required String transfermark, @Required BigInteger value) {
        subtractBalance(Msg.sender(), value);
        addBalance(to, value);
        emit(new TransferEvent(Msg.sender(),to,transfermark, value));
        return true;
    }
